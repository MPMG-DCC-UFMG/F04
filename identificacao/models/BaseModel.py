...
class BaseModel:
    def __init__(self,  embeddings_index, word_embed_dim=300, max_seq_length=50, num_folds=10,
                 ...
                 plots_folder=None, seed=42, text_column='text', label_column='label', name='BaseModel', initialize_weights_with='word2vec'):
        ...
        self.FILE_CACHE = {}
        self.name = name
        self.initialize_weights_with = initialize_weights_with

        if initialize_weights_with == 'bert':
            log('Loading neuralmind/bert-base-portuguese-cased...')

            self.tokenizer = AutoTokenizer.from_pretrained(
                'neuralmind/bert-base-portuguese-cased', do_lower_case=False)

            log('neuralmind/bert-base-portuguese-cased loaded!')
    ...
    def get_embedding_matrix(self):
        if self.initialize_weights_with == 'word2vec':
            return self.get_word2vec_matrix()
        if self.initialize_weights_with == 'bert':
            return self.get_bert_matrix()
    ...
    def preprocessing(self, files, sep=';', randomize=True):
        ...
        if self.initialize_weights_with == 'bert':
            return self.bert_preprocessing(files, sep, randomize)
    ...
    def bert_preprocessing(self, files, sep=';', randomize=True):
        log('Reading dataset from files')
        X, Y = self.generate_documents(self.folds_folder, files, sep=sep)
        ...
        log('Padded bert shape: %s' % str(np.array(padded).shape))

        attention_mask = np.where(padded != 0, 1, 0)
        log('Attention mask bert shape: %s' % str(attention_mask.shape))

        input_ids = torch.tensor(padded)

        attention_mask = torch.tensor(attention_mask)

        log('extracting features from the last hidden layer in the bert model...')
        with torch.no_grad():
        ...
        labels = np.array([1 if y == 'politics' else 0 for y in Y])

        log('labels shape: %s' % str(np.array(labels).shape))

        if self.model_name != 'CNN':
            features = last_hidden_states[0][:, 0, :].numpy()
            log('features shape: %s' % str(features.shape))

            self.word_embed_dim = features.shape[1]
            #self.max_seq_length = features.shape[1]
            self.vocab = [0] * (features.shape[0] - 1)
        else:
            features = last_hidden_states[0][:, :, :].numpy()
            log('features shape: %s' % str(features.shape))

            self.word_embed_dim = features.shape[2]
            #self.max_seq_length = features.shape[1]
            self.vocab = [0] * (features.shape[0] - 1)

            self.embeddings_index = features
            if self.pad_sequences:
                print('[ >> ] Pad documents...')
                features = pad_sequences(
                    features, maxlen=self.max_seq_length, padding="post", truncating="post", value=0)
