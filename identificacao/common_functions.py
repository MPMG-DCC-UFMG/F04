    ...
    try:
        if producer_instance is not None:
            try:
                ...
                print("Erro em publish_kafka_message: ", ex, flush=True)
                print(f'{ex}.\nException in publishing message.', flush=True)
...
class KafkaInteractor:
    ...
    def __get_valid_consumers(self, target_networks):
        ...
        'read_topics' no configurations.json... Delete as redes que não são target do atributo (já que o atributo é centralizado)
